...
export class FrameRenderer {
        ...
        private hasMatchingWebcamLayout(nextLayout: WebcamLayoutCache): boolean {
                ...
                return (
                        ...
                        areNearlyEqual(previousLayout.webcamWidth, nextLayout.webcamWidth) &&
                        areNearlyEqual(previousLayout.webcamHeight, nextLayout.webcamHeight) &&
        ...
        private applyWebcamLayout(nextLayout: WebcamLayoutCache): void {
                ...
                applyCoverLayoutToSprite(
                        ...
                        nextLayout.webcamWidth,
                        nextLayout.webcamHeight,
                        nextLayout.webcamWidth / 2,
                        nextLayout.webcamHeight / 2,
                        nextLayout.mirror,
                );
                ...
                drawSquircleOnGraphics(this.webcamMaskGraphics, {
                        x: 0,
                        y: 0,
                        width: nextLayout.webcamWidth,
                        height: nextLayout.webcamHeight,
                        radius: nextLayout.radius,
                });
                this.webcamMaskGraphics.fill({ color: 0xffffff });

                const minDim = Math.min(nextLayout.webcamWidth, nextLayout.webcamHeight);
                for (const layer of this.webcamShadowLayers) {
                        ...
                        const offsetY = minDim * layer.offsetScale * nextLayout.shadowStrength;
                        this.rasterizeShadowLayer(layer, {
                                ...
                                blur: Math.max(0, minDim * layer.blurScale * nextLayout.shadowStrength),
                        });
                }
        ...
        private updateWebcamOverlay(referenceTimeSeconds = this.currentVideoTime): void {
                ...
                const { width: webcamWidth, height: webcamHeight } = getWebcamOverlaySizePxRect({
                ...
                const position = getWebcamOverlayPositionRect({
                        containerWidth: this.config.width,
                        containerHeight: this.config.height,
                        webcamWidth,
                        webcamHeight,
