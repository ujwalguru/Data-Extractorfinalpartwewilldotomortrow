...
import {
        ...
        getWebcamOverlayPositionRect,
        getWebcamOverlaySizePxRect,
} from "./webcamOverlay";
...
const VideoPlayback = forwardRef<VideoPlaybackRef, VideoPlaybackProps>(
        (
                ...
                const webcamAspectRatio = webcam?.webcamAspectRatio ?? 1;
                ...
                const applyWebcamBubbleLayout = useCallback(
                        (zoomScale: number) => {
                                ...
                                const { width: scaledWidth, height: scaledHeight } = getWebcamOverlaySizePxRect({
                                ...
                                const { x, y } = getWebcamOverlayPositionRect({
                                        containerWidth: overlay.clientWidth,
                                        containerHeight: overlay.clientHeight,
                                        webcamWidth: scaledWidth,
                                        webcamHeight: scaledHeight,
                                        margin: webcamMargin,
                                ...
                                bubble.style.filter = `drop-shadow(0 ${Math.round(minDim * 0.06)}px ${Math.round(
                                        minDim * 0.22,
                ...
                useEffect(() => {
                        ...
                        const ticker = () => {
                                ...
                                // Apply 3D perspective CSS tilt when region has effect3d enabled
                                const effect3d = region?.effect3d as ZoomRegion3DEffect | undefined;
                                const cssTarget = containerRef.current;
                                if (cssTarget) {
                                        if (effect3d?.enabled && targetScaleFactor > 1.01) {
                                                const rawP = Math.min(1, Math.max(0,
                                                        (appliedScale - 1) / Math.max(0.001, targetScaleFactor - 1),
                                                ));
                                                // Smoothstep easing for a cinematic feel
                                                const normP = rawP * rawP * (3 - 2 * rawP);

                                                const intensity = effect3d.intensity ?? 0.5;
                                                const tiltX = effect3d.tiltX ?? 0;
                                                const tiltY = effect3d.tiltY ?? 0;

                                                // Max rotation scales with intensity (up to 22° at full)
                                                const maxDeg = intensity * 22;
                                                const rotYDeg = maxDeg * tiltX * normP;
                                                const rotXDeg = -maxDeg * tiltY * normP;

                                                // Cinematic perspective: stage-relative so it scales with output size
                                                const stageDim = Math.min(
                                                        stageSizeRef.current.width || 1280,
                                                        stageSizeRef.current.height || 720,
                                                );
                                                // More intensity → closer perspective → more dramatic depth
                                                const perspectivePx = stageDim * (3.2 - intensity * 1.8);

                                                // Pivot from the actual zoom focus point for natural tilt
                                                const focusXPct = ((targetFocus.cx ?? 0.5) * 100).toFixed(1);
                                                const focusYPct = ((targetFocus.cy ?? 0.5) * 100).toFixed(1);

                                                // Dynamic shadow shifts in the opposite direction of the tilt
                                                // to reinforce the 3D depth illusion
                                                const shadowShiftX = (-tiltX * normP * intensity * 18).toFixed(1);
                                                const shadowShiftY = (tiltY * normP * intensity * 18).toFixed(1);
                                                const shadowBlur = (normP * intensity * 40 + 6).toFixed(1);
                                                const shadowOpacity = (normP * intensity * 0.45).toFixed(3);

                                                cssTarget.style.transform = `perspective(${perspectivePx.toFixed(0)}px) rotateY(${rotYDeg.toFixed(3)}deg) rotateX(${rotXDeg.toFixed(3)}deg)`;
                                                cssTarget.style.transformOrigin = `${focusXPct}% ${focusYPct}%`;
                                                cssTarget.style.filter = `drop-shadow(${shadowShiftX}px ${shadowShiftY}px ${shadowBlur}px rgba(0,0,0,${shadowOpacity}))`;
                                                cssTarget.style.willChange = "transform, filter";
                                        } else {
                                                cssTarget.style.transform = "";
                                                cssTarget.style.transformOrigin = "center center";
                                                cssTarget.style.filter = "";
                                                cssTarget.style.willChange = "";
                                        }
                                }
                                ...
                                {pixiReady && videoReady && (
                                        ...
                                        >
                                                <div
                                                        ...
                                                        className="absolute rounded-md border border-[#a0673a]/80 bg-[#a0673a]/20 shadow-[0_0_0_1px_rgba(160,103,58,0.35)]"
