...
export function getWebcamOverlaySizePxRect({
        containerWidth,
        containerHeight,
        sizePercent,
        margin,
        ...
        const safeMargin = Math.max(0, margin);
        const maxWidth = Math.max(MIN_WEBCAM_OVERLAY_SIZE_PX, containerWidth - safeMargin * 2);
        const rawWidth = height * safeAspect;
        const width = Math.min(maxWidth, Math.max(MIN_WEBCAM_OVERLAY_SIZE_PX, rawWidth));
        return { width, height };
}

export function getWebcamOverlayPositionRect({
        containerWidth,
        containerHeight,
        webcamWidth,
        webcamHeight,
        ...
        const safeMargin = Math.max(0, margin);
        const availableWidth = Math.max(0, containerWidth - webcamWidth - safeMargin * 2);
        const availableHeight = Math.max(0, containerHeight - webcamHeight - safeMargin * 2);
        const presetPosition =
                positionPreset === "custom"
                        ? { x: clamp(positionX, 0, 1), y: clamp(positionY, 0, 1) }
                        : getWebcamPositionForPreset(positionPreset || legacyCorner);
