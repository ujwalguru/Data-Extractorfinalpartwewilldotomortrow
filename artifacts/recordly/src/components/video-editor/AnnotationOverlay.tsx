...
export function AnnotationOverlay({
	...
	const renderArrow = () => {
		...
		const color = annotation.figureData?.color || "#a0673a";
	...
	return (
		<Rnd
			...
			className={cn(
				...
				isSelected && "ring-2 ring-[#a0673a] ring-offset-2 ring-offset-transparent",
			)}
			style={{
				zIndex: isSelectedBoost ? zIndex + 1000 : zIndex, // Boost selected annotation to ensure it's on top
				pointerEvents: "auto",
				border: isSelected ? "2px solid rgba(160, 103, 58, 0.8)" : "none",
				backgroundColor: isSelected ? "rgba(160, 103, 58, 0.1)" : "transparent",
				boxShadow: isSelected ? "0 0 0 1px rgba(160, 103, 58, 0.35)" : "none",
			}}
			...
			resizeHandleStyles={{
				topLeft: {
					...
					border: isSelected ? "2px solid #a0673a" : "none",
					borderRadius: "50%",
					left: "-6px",
					top: "-6px",
					cursor: "nwse-resize",
